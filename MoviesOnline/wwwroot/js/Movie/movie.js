...
$("#uploadTrailer").click(function () {
    ...
    var files = $("#trailerUpload").get(0);
});
$("#uploadTrailer").click(function () {
    //console.log("#upload click");
    var data = new FormData();
    var files = $("#trailerUpload").get(0).files;

    if (files.length > 0) {
        data.append("UploadedTrailer", files[0]);
        //console.log(data);
    }
    $.ajax({
        type: "POST",
        url: "/Movies/UploadTrailer",
        data: data,
        contentType: false,
        processData: false,
        success: function (data) {
            //console.log(data.dbPath);
            $("#TrailerURL").val(data.dbPath);
            alert("Trailer is successfully uploaded!");
        },
        error: function () {
            alert("Error Uploading Trailer!");
        }
    });
});
